    ...
    stage('Publish github release') {
        if (!GIT_TAG) {
            ...
            withCredentials([[$class: 'StringBinding', credentialsId: 'GithubToken', variable: 'GITHUB_TOKEN']]) {
              sh "github-release info -u NinuxBasilicata -r ninux-build-openwrt -t ${GIT_TAG} || github-release release -u NinuxBasilicata -r ninux-build-openwrt -t ${GIT_TAG}"
              sh "github-release upload -u NinuxBasilicata -r ninux-build-openwrt -t ${GIT_TAG} -n ${ARCHIVE_NAME} -f ${UPLOAD_FILE}"
            }
        }
    }
}
